...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function createRandomBubbles(count) {
        for (let i = 0; i < count; i++) {
            ...
            const size = Math.random() * 10 + 350;
            const x = Math.random() * window.innerWidth;
            const y = Math.random() * document.body.scrollHeight;
            const duration = Math.random() * 15 + 5;
            bubble.style.width = `${size}px`;
            bubble.style.height = `${size}px`;
            bubble.style.background = "radial-gradient(circle, rgba(255, 255, 255, 0.39) 0%, rgba(36, 165, 160, 0.44) 100%)";
...
async function changeLanguage(lang) {
    document.querySelectorAll("[data-lang]").forEach(async (el) => {
        ...
        let newText = lang === "en" ? originalText : await translateText(originalText, lang);
...
function showProject(category, projectId) {
    ...
    const categoryContainer = category === "ui" ? document.querySelector(".btn-work").nextElementSibling.querySelectorAll(".project-image") : document.querySelector(".btn-work-web").nextElementSibling.querySelectorAll(".project-image");
    ...
    const buttonContainer = category === "ui" ? section.querySelectorAll(".btn-work button") : section.querySelectorAll(".btn-work-web button");
    ...
    event.target.style.background = "linear-gradient(to top, rgba(0, 0, 0, 0) -10%, rgb(255, 255, 255) 50%, rgba(0, 0, 0, 0) 110%)";
    event.target.style.boxShadow = "0px 0px 10px 2px rgba(125, 125, 125, 0.468)";
    event.target.style.transform = "scale(1.1)";
    event.target.style.color = "rgb(57, 153, 170)";
}
